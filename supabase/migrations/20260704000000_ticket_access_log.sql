-- HIPAA access-audit trail: one row per ticket/PHI access through the portal
-- (pull, sync, search, conversation fetch, coverage scan). Inserts happen only
-- server-side via the service role; admins can read.
