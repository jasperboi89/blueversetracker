...
export type SnipAttach =
  | { kind: "reason"; reasonId: string }
  | { kind: "section"; key: "phone" | "repeat" | "saveSummary" }
  | undefined;

export function AddSnipModal({
  sessionId, open, onOpenChange, attach,
}: { sessionId: string; open: boolean; onOpenChange: (v: boolean) => void; attach?: SnipAttach }) {
  const [name, setName] = useState("");
  const [category, setCategory] = useState<DispatchSnipCategory>("Other");
  ...
  const save = () => {
    if (!name.trim()) return;
    dispatchStore.addSnip(sessionId, {
      name: name.trim(), category, label: label.trim() || undefined,
      dataUrl: dataUrl ?? undefined, isImage,
    }, attach);
