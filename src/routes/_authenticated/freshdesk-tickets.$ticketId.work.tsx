...
function WorkspacePage() {
  ...
  const session: WorkSession =
    workSessions[ticketId] ?? ticketsStore.getSession(ticketId);
    ...
    return (
      <div className="mx-auto max-w-2xl pt-16 text-center">
        <div className="glass-panel p-8">
          ...
          <Button asChild className="mt-4"><Link to="/freshdesk-tickets">Back to Freshdesk Tickets</Link></Button>
        </div>
      </div>
    );
  }
  ...
  const generate = (force = false) => {
    ...
    if ((session.resultStatus === "waiting-cs" || session.resultStatus === "waiting-prog") && !session.waitingReason.trim()) {
  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-5">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            <Button size="sm" variant="ghost" asChild>
              <Link to="/freshdesk-tickets"><ArrowLeft className="mr-1 h-3.5 w-3.5" /> Back to Freshdesk Tickets</Link>
            </Button>
            ...
            <Button
              ...
              style={{
                background: "linear-gradient(110deg, oklch(0.4 0.18 290 / 0.55), oklch(0.4 0.16 240 / 0.7))",
      ...
      <SectionCard
        ...
        highlight={highlightSection === "issue"}
        innerRef={(el) => (sectionRefs.current.issue = el)}
      >
        <Textarea
          rows={5}
      ...
      <SectionCard
        ...
        highlight={highlightSection === "changes"}
        innerRef={(el) => (sectionRefs.current.changes = el)}
      >
        <Textarea
          rows={5}
      ...
      <SectionCard
        ...
        highlight={highlightSection === "result"}
        innerRef={(el) => (sectionRefs.current.result = el)}
      >
        <div className="space-y-3">
          ...
          {session.resultStatus === "failed" && (
            <div>
              ...
              <Input value={session.failureReason} onChange={(e) => update({ failureReason: e.target.value })} className="mt-1" placeholder="Required for Failed" />
            </div>
          )}
          {(session.resultStatus === "waiting-cs" || session.resultStatus === "waiting-prog") && (
            <div>
              <label className="text-xs uppercase tracking-wider text-muted-foreground">Waiting Reason *</label>
              <Input value={session.waitingReason} onChange={(e) => update({ waitingReason: e.target.value })} className="mt-1" placeholder="Required while waiting" />
            </div>
          )}
          <div>
            <label className="text-xs uppercase tracking-wider text-muted-foreground">Testing / Result Notes</label>
            <Textarea value={session.resultNotes} onChange={(e) => update({ resultNotes: e.target.value })} rows={3} className="mt-1" />
          </div>
        </div>
      </SectionCard>
      ...
      <SectionCard
        ...
        innerRef={(el) => (sectionRefs.current.notes = el)}
      >
        <div className="space-y-3">
          <div className="flex gap-2">
            <Textarea value={noteDraft} onChange={(e) => setNoteDraft(e.target.value)} rows={2} placeholder="Add a Hub note (LTP, Central timestamp)." />
            <Button
              onClick={() => {
          ...
          <div className="space-y-2">
            {ticket.hubHistory.filter((h) => h.kind === "note").map((h) => (
              <div key={h.id} className="rounded-md border border-border/30 bg-white/[0.02] p-3 text-sm">
      ...
      <SectionCard
        ...
        title="Snips"
        chip={snipCountChip}
        open={!!openSections.snips}
        onToggle={() => toggle("snips")}
        innerRef={(el) => (sectionRefs.current.snips = el)}
      >
        <div className="space-y-3">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {ticket.hubSnips.map((s) => (
              <div key={s.id} className="rounded-md border border-border/30 bg-white/[0.02] p-3 text-xs">
                {s.dataUrl && s.isImage ? (
                ...
                <div className="text-muted-foreground">{s.category}{s.label ? ` · ${s.label}` : ""}</div>
                <div className="text-[10px] text-muted-foreground">{formatCentralShort(new Date(s.createdAt))} · {s.initials}</div>
                <div className="mt-2 flex flex-wrap gap-1">
                  {s.dataUrl && (
                    <>
                      <Button size="sm" variant="ghost" onClick={() => navigator.clipboard.writeText(s.dataUrl!)}>
                        <Copy className="h-3 w-3" />
                      </Button>
                      <a href={s.dataUrl} download={s.name} className="inline-flex items-center rounded-md px-2 py-1 text-xs hover:bg-white/5">
                        <Download className="h-3 w-3" />
                      </a>
                      <a href={s.dataUrl} target="_blank" rel="noreferrer" className="inline-flex items-center rounded-md px-2 py-1 text-xs hover:bg-white/5">
                        <ExternalLink className="h-3 w-3" />
                      </a>
                    </>
                  )}
                  <Button size="sm" variant="ghost" onClick={() => ticketsStore.removeSnip(ticketId, s.id)}>
      ...
      <SectionCard
        ...
        innerRef={(el) => (sectionRefs.current.generate = el)}
      >
        <div className="space-y-3">
          <div className="grid gap-2 sm:grid-cols-2">
            <div>
              <label className="text-xs uppercase tracking-wider text-muted-foreground">Issue Classification</label>
              <Select
            ...
            <div>
              <label className="text-xs uppercase tracking-wider text-muted-foreground">Note Template</label>
              <Select
          ...
          {(ticket.issueClassification === "Scripting Issue" || ticket.issueClassification === "Client Change") && (
          ...
          {missing && (
            <div
              ...
              style={{
                ...
                background: "linear-gradient(120deg, oklch(0.85 0.16 85 / 0.12), oklch(0.85 0.16 210 / 0.08))",
              }}
            >
              <div className="font-medium text-foreground">Missing Details</div>
              <div className="mt-1 text-xs text-muted-foreground">Before generating, this ticket may need:</div>
              <ul className="mt-1 text-xs text-foreground/90">{missing.map((m) => <li key={m}>• {m}</li>)}</ul>
              <div className="mt-2 flex flex-wrap gap-1.5">
                {missing.includes("Ticket Issue") && <Button size="sm" variant="ghost" onClick={() => open("issue")}>Go to Ticket Issue</Button>}
                {missing.includes("Changes Made") && <Button size="sm" variant="ghost" onClick={() => open("changes")}>Go to Changes Made</Button>}
                {missing.includes("Result / Testing") && <Button size="sm" variant="ghost" onClick={() => open("result")}>Go to Result / Testing</Button>}
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            }}>Download Included Snips</Button>
            <Button size="sm" variant="ghost" onClick={() => window.open(ticket.details.freshdeskUrl, "_blank")}><ExternalLink className="mr-1 h-3.5 w-3.5" /> Open Freshdesk Ticket</Button>
          ...
          <div>
            ...
            {session.notePosted && session.notePostedAt && (
              <div className="mt-1 text-[10px] text-muted-foreground">
                Marked posted manually at {formatCentralShort(new Date(session.notePostedAt))} · LTP · v{session.noteVersion}
              </div>
            )}
          </div>
        </div>
      ...
      <MarkPostedDialog open={postedOpen} onOpenChange={setPostedOpen} onConfirm={() => { ticketsStore.markPosted(ticketId); toast.success("Marked as posted."); }} />
      <MarkCompletedDialog
        open={completeOpen}
...
function MarkPostedDialog({ open, onOpenChange, onConfirm }: { open: boolean; onOpenChange: (v: boolean) => void; onConfirm: () => void }) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="glass-panel border-0 sm:max-w-md">
        <DialogHeader><DialogTitle>Mark this note as posted manually?</DialogTitle></DialogHeader>
...
function MarkCompletedDialog({
  open, onOpenChange, ticket, session, onComplete,
}: {
  open: boolean;
  ...
  useEffect(() => { if (open) setCls(ticket.issueClassification ?? ""); }, [open, ticket.issueClassification]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="glass-panel border-0 sm:max-w-md">
        <DialogHeader><DialogTitle>Mark Ticket Completed?</DialogTitle></DialogHeader>
        <div className="space-y-3">
          <div>
            <label className="text-xs uppercase tracking-wider text-muted-foreground">Issue Classification *</label>
            <Select value={cls} onValueChange={(v) => setCls(v as IssueClassification)}>
              <SelectTrigger className="mt-1"><SelectValue placeholder="Required" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="Scripting Issue">Scripting Issue</SelectItem>
                <SelectItem value="Client Change">Client Change</SelectItem>
                <SelectItem value="Other">Other</SelectItem>
          ...
          {warnings.length > 0 && (
            <div className="rounded-md border border-border/40 bg-white/[0.02] p-2 text-xs text-muted-foreground">
              ...
              <div className="mt-1 text-muted-foreground/70">These don't block completion — only Issue Classification is required.</div>
            </div>
          )}
        ...
        <DialogFooter>
          ...
          <Button
            disabled={!cls}
            onClick={() => { onComplete(cls as IssueClassification); onOpenChange(false); }}
